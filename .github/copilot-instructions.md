...
### 0. Language and Layout Lock

- All website-facing content must be English only for V1.2.
- Multilingual support is out of scope until the English baseline is complete.
- Keep the existing page skeleton and layout hierarchy intact.
- Prefer content, icon, and link updates over layout rewrites.
...
### 7. Metadata and Tracking Compliance

- Every new `.md` or `.mdx` document must include frontmatter taxonomy fields: `L1_Journey`, `L2_Role`, and `L3_Artifact`.
- All outbound CTAs from geodocs.dev to geolify.ai must include full UTM parameters:
  - `utm_source=geodocs.dev`
  - `utm_medium=referral`
  - `utm_campaign=brand_audit`
  - `utm_content=<placement>`
- Validate outbound geolify.ai links after implementation and before handing off.
